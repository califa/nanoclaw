...
function collectAgentGroups() {
  return getAllAgentGroups().map((g) => {
    ...
    const members = getMembers(g.id).map((m: { user_id: string }) => {
      const user = getUser(m.user_id);
      return { ...m, display_name: user?.display_name ?? null };
    });
    const admins = getAdminsOfAgentGroup(g.id).map((a: { user_id: string }) => {
    ...
    return {
      ...
      name: g.name,
      folder: g.folder,
      agent_provider: (g as unknown as { agent_provider?: string }).agent_provider ?? null,
      container_config: (g as unknown as { container_config?: string }).container_config
        ? JSON.parse((g as unknown as { container_config: string }).container_config)
        : null,
...
function collectUsers() {
  return getAllUsers().map((u: { id: string; kind: string; display_name: string | null; created_at: string }) => {
    ...
    let privilege = 'none';
    if (roles.some((r: { role: string }) => r.role === 'owner')) privilege = 'owner';
    else if (roles.some((r: { role: string; agent_group_id: string | null }) => r.role === 'admin' && !r.agent_group_id))
      privilege = 'global_admin';
    else if (roles.some((r: { role: string }) => r.role === 'admin')) privilege = 'admin';
    ...
    return {
      ...
      roles,
      memberships,
      dmChannels: dms.map((d: { channel_type: string }) => ({ channel_type: d.channel_type })),
